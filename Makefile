# This Makefile is for the Local::JSONParser extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Local/JSONParser.pm]
#     AUTHOR => [q[Mons <mons@macports.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Local::JSONParser]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Local/JSONParser.pm]
...
DIRFILESEP = \\
DFSEP = $(DIRFILESEP)
NAME = Local::JSONParser
NAME_SYM = Local_JSONParser
VERSION = 0
...
MAKE = dmake
FULLEXT = Local\JSONParser
BASEEXT = JSONParser
PARENT_NAME = Local
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Local/JSONParser.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
TO_INST_PM = lib/Local/JSONParser.pm
...
DISTNAME = Local-JSONParser
DISTVNAME = Local-JSONParser-0
...
JSONParser.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Local::JSONParser\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "  - 'Mons <mons@macports.org>'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Local-JSONParser" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "      \"Mons ^<mons^@macports.org^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Local-JSONParser\"," >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) "<SOFTPKG NAME=\"Local-JSONParser\" VERSION=\"0\">" > Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "    <ABSTRACT></ABSTRACT>" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Mons &lt;mons@macports.org&gt;</AUTHOR>" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "        <ARCHITECTURE NAME=\"MSWin32-x64-multi-thread-5.24\" />" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "        <CODEBASE HREF=\"\" />" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) "    </IMPLEMENTATION>" >> Local-JSONParser.ppd
	$(NOECHO) $(ECHO) ^</SOFTPKG^> >> Local-JSONParser.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/Local/JSONParser.pm blib\lib\Local\JSONParser.pm 
